                ...
                _.each(d['@graph'], function(elem) {
                    if (elem['@type'] === 'ao:Annotation') {
                        ...
                        if (typeof(elem['ao:body']) === 'string') {
                            d.data.push({id: id, tfidf: tfidf, cui: cui, sty: sty, group: group, term: [elem['ao:body']]});
                        } else {
                            d.data.push({id: id, tfidf: tfidf, cui: cui, sty: sty, group: group, term: elem['ao:body'].sort()});
                        }
                    }
                });
                return d;
